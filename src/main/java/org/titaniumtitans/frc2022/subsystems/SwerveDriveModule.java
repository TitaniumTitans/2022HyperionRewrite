...
/**
 * This class encapsulates the hardware and internal state of a
 * SwerveDriveSpecialties MK4i swerve module.
 */
public class SwerveDriveModule extends SubsystemBase {
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(m_drive.getSelectedSensorVelocity(),
                new Rotation2d(m_turning.getSelectedSensorPosition()));
    }

    public void setDesiredState(SwerveModuleState desiredState) {
        /*
         * Optimize the swerve module state so the wheel never has to rotate more than
         * 90 degrees
         */
        SwerveModuleState state = SwerveModuleState.optimize(desiredState,
                new Rotation2d(m_turning.getSelectedSensorPosition()));

        final double drivingOutput = (state.speedMetersPerSecond / (Constants.ModuleConstants.kWheelDiameterMeter / 2))
                * 321.6990877275948;
